...
public class ToolWeb {
	...
	/**
	 * 传入Controller获取客户端IP地址
	 * @param request
	 * @return
	 */
	public static String getIpAddr(Controller controller) {
		HttpServletRequest request = controller.getRequest();
		return getIpAddr(request);
	}
	@SuppressWarnings("rawtypes")
	public static String getParamString(Controller controller){
		StringBuffer buffer = new StringBuffer();
		Enumeration<String> paramNames = controller.getParaNames();
		String key = null, value = null;
		for(Enumeration e = paramNames; e.hasMoreElements();){
			key = e.nextElement().toString();
			value = controller.getPara(key);
	 ...
	 * @param maxAge		cookie存放的时间(以秒为单位,假如存放三天,即3*24*60*60; 如果值为0,cookie将随浏览器关闭而清除)
	 */
	public static void addCookie(Controller controller, 
			String domain, String path, boolean isHttpOnly, 
			String name, String value, int maxAge) {
		Cookie cookie = new Cookie(name, value);
		// 所在域：比如a1.4bu4.com 和 a2.4bu4.com 共享cookie
